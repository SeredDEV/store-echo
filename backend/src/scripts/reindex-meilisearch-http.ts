...
export default async function reindexMeilisearchHttp({ container }: ExecArgs) {
  try {
    ...
    if (!regionsResponse.ok) {
      throw new Error(
        `Error obteniendo regiones: ${regionsResponse.statusText}`
      );
    }
    ...
    if (regions.length === 0) {
      console.log(
        "⚠️  No se encontraron regiones. Creando productos vacíos..."
      );
      console.log(
    ...
    while (hasMore) {
      ...
      if (!productsResponse.ok) {
        throw new Error(
          `Error obteniendo productos: ${productsResponse.statusText}`
        );
      }
      ...
      console.log(
        `📦 Obtenidos ${products.length} productos (total: ${allProducts.length}/${count})`
      );
