...
export default async function reindexMeilisearchDirect({
  container,
}: ExecArgs) {
  try {
    ...
    try {
      ...
      if (typeof (productModuleService as any).listProducts === "function") {
        ...
        allProducts =
          (await (productModuleService as any).listProducts(
            {},
            {
      ...
      // Método 2: Intentar con listAndCount
        ...
        typeof (productModuleService as any).listAndCount === "function"
      ) {
      ...
      // Método 3: Intentar con query
        ...
        allProducts =
          (await (productModuleService as any).query(
            {},
            {
              relations: ["variants", "images", "categories", "tags"],
            }
        ...
        console.log(
          "💡 Para indexar productos existentes, actualízalos desde el admin de Medusa."
        );
        return;
      }
    } catch (error: any) {
    ...
    if (allProducts.length === 0) {
      ...
      console.log(
        "✅ Índice creado. Los productos se indexarán cuando se creen."
      );
      return;
    }
    ...
    // Indexar productos
    console.log(
      `📝 Indexando ${searchableProducts.length} productos en Meilisearch...`
    );
    await meilisearchService.reindexAll(searchableProducts, "product");

    console.log(
      `✅ Reindexación completada: ${searchableProducts.length} productos indexados`
    );
  } catch (error: any) {
